...
func CalcDifficulty(config *params.ChainConfig, time uint64, parent *types.Header) *big.Int {
	...
	case config.IsEthPoWFork(next):
		if config.HasEthPoWTransitionReset() {
			if big.NewInt(0).Add(config.EthPoWForkBlock, big.NewInt(2048)).Cmp(next) == 0 {
				return params.ETHWStartDifficulty // Reset difficulty for the legacy ETHW fork transition.
			}
...
func (ethash *Ethash) Prepare(chain consensus.ChainHeaderReader, header *types.Header) error {
	...
	if activation == nil || ethash.config.PowMode == ModeFake || ethash.config.PowMode == ModeFullFake {
...
func (ethash *Ethash) accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) error {
	...
	activation, err := config.USDBActivationAt(header.Number.Uint64())
	if err != nil {
		return fmt.Errorf("invalid usdb consensus config: %w", err)
	}
	if activation == nil {
	...
	return ethash.accumulateUSDBRewards(config, activation, state, header, uncles)
}
