...
对于 USDB 新链，手续费分账目标地址不建议依赖“链启动后再部署合约”来确定，否则会形成循环依赖：

- 共识规则需要先知道手续费应该打到哪个地址
- 但普通运行时部署又依赖链已经启动并正常出块

第一阶段最稳妥的方案是：

- 选定一个 **系统保留地址** 作为 `DividendAddress`
- 该地址不作为普通用户地址使用
- 在 **genesis** 中直接预置该地址的合约 `code`
- 若分红池逻辑需要初始状态，再一并预置 `storage`
- 手续费分账从 `block 0` 开始直接打到该地址

这样做的好处是：

- 没有冷启动循环依赖
- 地址从 genesis 起固定，节点间不存在歧义
- 不需要依赖运行时部署流程
- 后续如果要升级分红池逻辑，也可以继续围绕同一地址做 fork 升级或合约升级设计

不推荐第一阶段采用：

- 先用普通 EOA 地址收款，之后再尝试“升级”为合约地址
- 依赖 `CREATE2` 先预承诺地址、再在链启动后补部署

这些路径都比“系统保留地址 + genesis 预置合约”更复杂，也更容易引入运维和一致性风险。
...
4. genesis 预置合约的最小代码形态与是否需要初始 storage
5. 分账比例是否会分叉升级
6. 是否需要补一个专项测试矩阵，覆盖：
   - pre-London
   - post-London
